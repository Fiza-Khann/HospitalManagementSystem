...
namespace HMS_Task1
{
    public partial class AttendanceLog : System.Web.UI.Page
    {
        ...
        // Event handler for deleting selected records
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            ...
            foreach (GridViewRow row in MainGrid.Rows)
            {
                ...
                {
                    recordSelected = true;
                    string AttendanceID = MainGrid.DataKeys[row.RowIndex].Value.ToString();
        ...
        private void DeleteAttendance(string AttendanceID)
        {
            ...
            {
                using (SqlCommand cmd = new SqlCommand("DELETE FROM Attendance WHERE ID = @ID", conn))
                {
        ...
        // Event handler for updating selected records
        protected void btnUpdate_Click(object sender, EventArgs e)
        {
            foreach (GridViewRow row in MainGrid.Rows)
            {
                ...
                if (chkSelect != null && chkSelect.Checked)
                {
                    string originalAttendanceID = MainGrid.DataKeys[row.RowIndex].Value.ToString();

                    // Redirect to EditAttendanceLog.aspx with the original AttendanceID
                    Response.Redirect("AttendanceForm.aspx?ID=" + originalAttendanceID);
                    return;
                }
            }
