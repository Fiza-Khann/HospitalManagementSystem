...
{
    public partial class IPDEdit_MedServices : System.Web.UI.Page
    {
        ...
        private string GenerateUniqueID()
        {
            ...
            {
                string query = "SELECT ISNULL(MAX(ID), 0) + 1  FROM IPD_MedServices";
        ...
        private void LoadXrayDetails(string id)
        {
            using (SqlConnection conn = new SqlConnection(connectionString))
            {
                try
                {
                    string query = "SELECT * FROM IPD_MedServices WHERE ID = @ID";
                    SqlCommand cmd = new SqlCommand(query, conn);
            ...
            if (Page.IsValid)
            {
                try
                {
                    using (SqlConnection conn = new SqlConnection(connectionString))
                    {
                        ...
                        query = "SELECT COUNT(*) FROM IPD_MedServices WHERE ID = @ID";
                        SqlCommand cmd = new SqlCommand(query, conn);
