...
{
    public partial class IPDEdit_Ultrasound : System.Web.UI.Page
    {
        ...
        private string GenerateUniqueID()
        {
            ...
            {
                string query = "SELECT ISNULL(MAX(ID), 0) + 1  FROM IPD_Ultrasound";
        ...
        private void LoadXrayDetails(string id)
        {
            using (SqlConnection conn = new SqlConnection(connectionString))
            {
                try
                {
                    string query = "SELECT * FROM IPD_Ultrasound WHERE ID = @ID";
                    SqlCommand cmd = new SqlCommand(query, conn);
            ...
            if (Page.IsValid)
            {
                try
                {
                    using (SqlConnection conn = new SqlConnection(connectionString))
                    {
                        ...
                        query = "SELECT COUNT(*) FROM IPD_Ultrasound WHERE ID = @ID";
                        SqlCommand cmd = new SqlCommand(query, conn);
