...
{
    public partial class IPDView_MedServices : System.Web.UI.Page
    {
        ...
        private void BindGrid()
        {
            ...
            try
            {
                ...
                {
                    using (SqlCommand cmd = new SqlCommand("SELECT * FROM IPD_MedServices", con))
                    {
        ...
        private void DeleteConsultant(string id)
        {
            string constr = ConfigurationManager.ConnectionStrings["HMS"].ConnectionString;
            using (SqlConnection conn = new SqlConnection(constr))
            {
                using (SqlCommand cmd = new SqlCommand("DELETE FROM IPD_MedServices WHERE ID = @ID", conn))
                {
        ...
        protected void btnUpdate_Click(object sender, EventArgs e)
        {
            foreach (GridViewRow row in MainGrid.Rows)
            {
                ...
                if (chkSelect != null && chkSelect.Checked)
                {
                    ...
                    Response.Redirect("IPDEdit_MedServices.aspx?originalReceiptNo=" + originalReceiptNo);
                    return;
                }
            }
