...
function ShiftCharts(FromDate, ToDate) {
    catName = [];
    $.ajax({
        ...
        data: { DTfrom: FromDate, DTTo: ToDate },
        dataType: 'json',
        success: function (response) {
            var series = new Array();
            for (var i in response.ChartList) {
                var serie = new Array(response.ChartList[i].CatName, response.ChartList[i].Total);
                series.push(serie);
                catName.push(response.ChartList[i].CatName)
            }
            var chartTitle = response.SubHeading;
            DrawStatusArea('MilkCatchart', series, catName, chartTitle, 'column');
...
function DrawStatusArea(id, series, catName, chartTitle, chartType) {
    CatNames = [];
    Morning = [];
    Afternoon = [];
    Evening = [];
                ...
                point: {
                    events: {
                        click: function () {
                            ...
                            $("#hdDTFrom").val($("#DTfrom").val());
                            $("#hdDTTo").val($("#DTTo").val());
                            $("#hdCatName").val(this.category);
                            $("#hdShift").val(this.name);
