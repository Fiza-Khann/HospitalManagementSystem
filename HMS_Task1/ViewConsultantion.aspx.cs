...
namespace HMS_Task1
{
    public partial class ViewConsultantion : System.Web.UI.Page
    {
        ...
        private void BindGrid()
        {
            ...
            try
            {
                ...
                {
                    using (SqlCommand cmd = new SqlCommand("SELECT * FROM OP_Consultantion", con))
                    {
        ...
        private void DeleteConsultant(string ReceiptNo)
        {
            ...
            {
                using (SqlCommand cmd = new SqlCommand("DELETE FROM OP_Consultantion WHERE ReceiptNo = @ReceiptNo", conn))
                {
        ...
        protected void btnUpdate_Click(object sender, EventArgs e)
        {
            foreach (GridViewRow row in MainGrid.Rows)
            {
                ...
                if (chkSelect != null && chkSelect.Checked)
                {
                    ...
                    Response.Redirect("AddConsultantion.aspx?originalReceiptNo=" + originalReceiptNo);
                    return;
                }
            }
