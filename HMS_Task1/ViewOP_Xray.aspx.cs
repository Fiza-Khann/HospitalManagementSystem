...
namespace HMS_Task4
{
    public partial class ViewOP_Xray : System.Web.UI.Page
    {
        ...
        private void BindGrid()
        {
            ...
            try
            {
                ...
                {
                    using (SqlCommand cmd = new SqlCommand("SELECT * FROM OP_Xray", con))
                    {
        ...
        private void DeleteConsultant(string ReceiptNo)
        {
            ...
            {
                using (SqlCommand cmd = new SqlCommand("DELETE FROM OP_Xray WHERE ReceiptNo = @ReceiptNo", conn))
                {
        ...
        protected void btnUpdate_Click(object sender, EventArgs e)
        {
            foreach (GridViewRow row in MainGrid.Rows)
            {
                ...
                if (chkSelect != null && chkSelect.Checked)
                {
                    ...
                    Response.Redirect("EditOP_Xray.aspx?originalReceiptNo=" + originalReceiptNo);
                    return;
                }
            }
